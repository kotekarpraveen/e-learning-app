...
export const AdminTeam: React.FC = () => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const res = await api.saveTeamMember(newMember);

        if (res.success) {
            success(modalMode === 'add' ? 'Team member added successfully' : 'Team member updated successfully');
            setIsModalOpen(false);
            fetchData(); // Reload to get real records
        } else {
            error("Failed to save team member: " + res.message);
        }
    };
    ...
    const handleDelete = async (id: string) => {
        if (confirm("Are you sure you want to remove this team member? This action cannot be undone.")) {
            ...
            if (res.success) {
                ...
                error("Failed to delete member: " + res.message);
            }
        }
    };
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filteredTeam.map(member => (
                                <tr key={member.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold capitalize ${member.role === 'super_admin' ? 'bg-purple-100 text-purple-700' :
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${member.status === 'Active' ? 'text-green-700 bg-green-50' : 'text-gray-500 bg-gray-100'
                                            }`}>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-5 gap-3">
                                            {[
                                                ...
                                                <div
                                                    ...
                                                    className={`cursor-pointer rounded-xl border p-3 text-center transition-all ${formData.role === roleOpt.id
                                                    ...
                                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                        {group.permissions.map(perm => {
                                                            ...
                                                            return (
                                                                <label
                                                                    key={perm.code}
                                                                    className={`flex items-start p-2 rounded-lg transition-colors ${isDisabled ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer hover:bg-gray-50'
                                                                        }`}
                                                                >
