...
const Toast = ({ message, type, onClose }: { message: string, type: 'success' | 'error', onClose: () => void }) => {
    ...
    return (
        <MotionDiv
            ...
            className={`fixed bottom-6 right-6 z-50 px-6 py-4 rounded-xl shadow-2xl flex items-center space-x-3 border ${type === 'success' ? 'bg-white border-green-100' : 'bg-white border-red-100'
                }`}
        >
                        ...
                        {contentLibrary.map(asset => (
                            <div key={asset.id} className="p-4 flex items-center justify-between hover:bg-gray-50 transition-colors group">
                                <div className="flex items-center gap-4">
                                    <div className={`p-2 rounded-lg border bg-white ${asset.type.includes('Video') ? 'text-blue-600 border-blue-100' :
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-200 space-y-6 shadow-sm">
                    ...
                    <div className="flex gap-4">
                        {['public', 'private', 'unlisted'].map(v => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg border text-sm font-medium capitalize ${settings.visibility === v
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-100">
                    ...
                    <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${settings.enableDiscussion ? 'bg-primary-600' : 'bg-gray-200'
                            }`}
                    >
                        <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${settings.enableDiscussion ? 'translate-x-6' : 'translate-x-1'
                            }`} />
                    </button>
                </div>
            </div>
        ...
        const fetchCourse = async () => {
            ...
            if (data) {
                ...
                setModules(data.modules.map(m => ({
                    id: m.id, title: m.title, description: m.description || '', isExpanded: false, isPodcast: m.isPodcast, lessons: m.lessons.map(l => {
