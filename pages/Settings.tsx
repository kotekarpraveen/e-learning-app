...
import { 
  ...
  Check, Shield, Mail, AlertCircle, X
} from 'lucide-react';
...
export const Settings: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    if (isSupabaseConfigured() && user) {
        ...
        // Only update bio if it has content (assuming column exists)
  ...
  const tabs = [
    ...
    ...(user?.role === 'admin' ? [{ id: 'billing', label: 'Billing', icon: <CreditCard size={18} /> }] : [])
  ];
                ...
                <div className="space-y-6">
                   ...
                   <div className="divide-y divide-gray-100">
                      <Toggle 
                        ...
                        description="Receive updates about your enrolled courses and new material."
                        checked={notifications.courseAnnouncements} 
                        onChange={v => setNotifications({...notifications, courseAnnouncements: v})}
                      />
                      <Toggle 
                        label="Security Alerts" 
                        description="Get notified about suspicious activity and logins."
                        checked={notifications.securityAlerts} 
                        onChange={v => setNotifications({...notifications, securityAlerts: v})}
                      />
                      <Toggle 
                        label="Marketing Emails" 
                        description="Receive tips, trends, and special offers."
