...
export const AdminInstructors: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    await api.saveInstructor(newInstructor);
    
    // Optimistic Update
    if (modalMode === 'add') {
        setInstructors([...instructors, newInstructor]);
    } else {
        setInstructors(instructors.map(i => i.id === newInstructor.id ? newInstructor : i));
    }
               ...
               <div className="p-6">
                  <div className="flex justify-between items-start mb-4">
                     ...
                     <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                        instructor.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'
                     }`}>
                        {instructor.status}
                     </span>
                  </div>
